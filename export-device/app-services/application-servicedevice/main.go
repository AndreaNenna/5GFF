...
type Sensor_Data struct {
	...
	LicenseRemaining string `json:"LicenseRemaining"`
}
...
//	fmt.Println("\n\n", "contenuto topic event", myJsonString)
	...
	fmt.Println(err)
        ...
        if JsonData.Device == "DISPOSITIVO_DEVICE_PRINTER"{
	        // Value e valueacc sono stringhe
	        fmt.Println("\n\n", "STRINGA JSONacc ISOLATO", JsonData.Readings[0].Value)
//       	 fmt.Println("\n\n", JsonData.Device)

        	valuestate1 := strings.NewReplacer("'", "\"")
        	valuestate := valuestate1.Replace(JsonData.Readings[0].Value)
//        	fmt.Println("\n\n", "STRINGA JSONPIR ISOLATO CON DOPPI APICI", valueacc)
        	...
        	ValueState := []byte(string(valuestate))
		...
		var ValueJsonData DeviceValue

		// Unmarshal the valueacc string in ValueJsonData
		Err := json.Unmarshal(ValueState, &ValueJsonData)
...
//        
        	...
        	data.Sn = ValueJsonData.Sn
        	data.Model = ValueJsonData.Model 
        	data.Type = ValueJsonData.Type
        	data.Material = ValueJsonData.Material
        	data.Shipping = ValueJsonData.Shipping
        	data.LicenseRemaining = ValueJsonData.LicenseRemaining
